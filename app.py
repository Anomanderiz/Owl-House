...
st.session_state.setdefault("ledger", pd.DataFrame(columns=[
...
st.session_state.setdefault("ward_focus", "Dock")  # ensure defined before any buttons that log
...
MURAL_B64 = load_b64_first("assets/mural_sidebar.png", "assets/mural.png")
MURAL_H, MURAL_OPACITY = 1120, 0.8
...
with c2:
    ...
    with btnL:
        if st.button("Lie Low (−1/−2 Heat)", key="btn_lie_low"):
            ...
            adj = [dt.datetime.now().isoformat(timespec="seconds"), st.session_state["ward_focus"],
...
with c3:
    st.selectbox("Active Ward", ["Dock","Field","South","North","Castle","Trades","Sea"], key="ward_focus")
...
with tab1:
    ...
    if st.button("Queue Mission → Resolve & Log", type="primary"):
        st.session_state._queued_mission = dict(
            ward=st.session_state["ward_focus"], archetype=arc, BI=BI, EB=EB, OQM=sum(OQM),
...
# ---------- Helpers for Wheel (cached for speed) ----------
@st.cache_data(show_spinner=False)
def _load_complications(heat_state: str):
    table_path = "assets/complications_high.json" if heat_state == "High" else "assets/complications_low.json"
    with open(table_path, "r") as f:
        return json.load(f)

@st.cache_data(show_spinner=False)
def _wheel_b64(n: int, size: int, colors=None, gold="#d0a85c", ivory="#eae7e1"):
    ...
    for i in range(n):
        ...
        d.text((tx, ty), str(i+1), fill=ivory, font=font, anchor="mm")
...
with tab3:
    ...
    options = _load_complications(heat_state)

    wheel_b64 = _wheel_b64(len(options), WHEEL_SIZE, gold=GOLD, ivory=IVORY)
    ...
    if hidden_clicked:
        ...
        try:
            ...
            row = [dt.datetime.now().isoformat(timespec="seconds"),
                   st.session_state["ward_focus"], "Complication", "-", "-", "-", 0, 0, "-", "-", comp]
            st.session_state.ledger.loc[len(st.session_state.ledger)] = row
        except Exception:
            pass
